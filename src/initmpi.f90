...
module mod_initmpi
  use mpi
  use decomp_2d
  use mod_common_mpi, only: myid,ierr,halo,canscomm,ipencil => ipencil_axis
  use mod_types
  !@acc use openacc
  !@acc use cudecomp
  ...
  subroutine initmpi(ng,dims,bc,lo,hi,n,n_x_fft,n_y_fft,lo_z,hi_z,n_z,nb,is_bound)
    ...
    !
#if defined(_OPENACC)
    call MPI_COMM_SPLIT_TYPE(canscomm,MPI_COMM_TYPE_SHARED,0,MPI_INFO_NULL,local_comm,ierr)
    ...
    !
    istat = cudecompInit(ch,canscomm)
    !
...
#endif
    call decomp_2d_init(ng(1),ng(2),ng(3),dims(1),dims(2),periods,canscomm)
