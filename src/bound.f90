...
module mod_bound
  use mpi
  use mod_common_mpi, only: ierr,halo,ipencil_axis,canscomm
  use mod_types
  use mod_opposition
  use mod_drl
  implicit none
  ...
  contains
  subroutine bounduvw(cbc,n,bc,nb,is_bound,is_correc,dl,dzc,dzf,u,v,w,pfix)
    !
    ...
    logical,  intent(in), optional :: pfix
    logical :: impose_norm_bc
    integer :: idir,nh,k,i,j
    !
    nh = 1
    !
#if !defined(_OPENACC)
    ...
    if ((impose_norm_bc).and.(is_bound(0,3)).and.(present(pfix))) then
      call opposition(n,alpha_opp,0,var_opp,w)
    endif

    if ((impose_norm_bc).and.(is_bound(1,3)).and.(present(pfix)).and.(n_walls.eq.2)) then
